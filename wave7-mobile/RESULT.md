...
HUD, toast, and modal pad with `env(safe-area-inset-*)` tokens. Canvas stays full-bleed (sky under the notch); chrome does not. Portrait adds a little extra above the home indicator so **Drive** and miles never sit under the system bar. Layout uses `100dvh`. Hit targets stay ≥48px (Drive, Hide, Sound, Larger type, Less motion). Capture: `?panel=1` at 390×844 with island + home-indicator overlays.

### 2. Short-phone sheet (~667)
Open **The car** at ~667 is a shorter sheet (`max-height: min(14vh, 5.85rem)`), upgrade blurbs tuck away, and Hide stays in the sheet header. Wagon roof, tailgate, and horizon stay in view with a full Drive (`min-height: max(48px, 3.15rem)`). No second bar on Drive. Collapsed default stands. Capture: `?panel=1` at 390×667.

### 3. Landscape thumb path
Two-column calm. The right column is capped (`min(15rem, 30vw)`) so upgrades do not cover the road corridor or wagon. **Drive** sits in the right thumb zone under the sheet. Miles sit bottom-left. Portrait stays the hero layout. Safe-area left/right apply (notch on the long edge). Disabled prestige tucks away so the panel is upgrades + Drive. Capture: landscape 844×390.

### 4. Mobile type hierarchy
Read order is **destination → miles → Drive → upgrades**. On phone, destination is the display line (full width, one line); “Sunday Drive” is a smaller nowrap wordmark. Comfort buttons wrap in two rows instead of a tall right stack. **Larger type** still steps up without wrapping Drive or crowding the postcard (capped miles / Drive on short and landscape). Cream-on-walnut held. Capture: `?panel=1&large=1` at 390×844.

### 5. Calm mobile perf
Skipped. Capture-loop frame time was ~13ms (~75fps) at the phone viewports. Existing `setPixelRatio` cap of 2 stands. No shadow or post-stack changes.
...
- `phone-landscape.png` — landscape, Drive in the right thumb zone, wagon/road clear
- `phone-390-large-type.png` — optional Larger type on 390; Drive still one line
- `phone-390-collapsed.png` — extra; collapsed default, miles + Drive
